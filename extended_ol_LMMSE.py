...
# MMSE estimtor based on two past channel outputs
...
def ozarow(message1, forward_noise1, feedback_noise1, message2, forward_noise2, feedback_noise2, args):
	...
	for t in range(2, args.N + 1):
		...
		if t == 2:
			...
			rhok_history = torch.cat([rhok_history, rho, rho], dim = 1)
			...
			update_estimate1 = 	(psi_kminus1 * torch.sqrt(alpha1) * (1 + g * torch.abs(rho))) * (pi1 * Y1t - lambda1 * Y1_history[:,-1].view(num_samples,1))/(pi1**2 - torch.square(lambda1)) 

			update_estimate2 = (psi_kminus1 * torch.sqrt(alpha2) * (g + torch.abs(rho)) * sgn(rho)) * (pi2 * Y2t - lambda2 * Y2_history[:,-1].view(num_samples,1))/ (pi2**2 - torch.square(lambda1))
			...
			alpha1, alpha2, rho = update(alpha1, alpha2, rho, lambda1, lambda2, P, g, sigma1, sigma2, rhoz,  pi1, pi2, psi_kminus1)
