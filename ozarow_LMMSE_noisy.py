# extend ol to noisy feedback case
...
def ozarow(message1, forward_noise1, feedback_noise1, message2, forward_noise2, feedback_noise2, args):
	...
	for t in range(2, args.N + 1):
		if t == 2:
			...
			alpha1_rec = torch.var(epsilon1)
			alpha2_rec = torch.var(epsilon2)
			...
			coe1 = (torch.sqrt(P/Dk) * torch.sqrt(alpha1_enc) * (1 + g * torch.abs(rho)))/(P + sigma1**2)
			coe2 = (torch.sqrt(P/Dk) * torch.sqrt(alpha2_enc) * (g + torch.abs(rho)) * sgn(rho))/(P + sigma2**2)
			...
			alpha1_enc, alpha2_enc, rho = update(alpha1_enc, alpha2_enc, rho, Dk, P, g, sigma1, sigma2)
			Dk = 1 + g**2 + 2 * g * torch.abs(rho)
