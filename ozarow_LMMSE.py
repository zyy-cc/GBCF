...
# Model: two-user Gaussian broadcast channel with noiseless feedback
# sigma^2 = 0, sigma_1^2 = sigma_2^2
# first two transmission: LMMSE estimator
...
def get_args(jupyter_notebook):
	...
	# channel definition
	parser.add_argument('-forward_SNR1', type=int, default=-1) # forward SNR for receiver 1
	parser.add_argument('-forward_SNR2', type=int, default=-1) # forward SNR for receiver 2
	parser.add_argument('-feedback_SNR1', type=int, default=100, help='100 means noiseless feeback') # feedback SNR for receiver 1
	parser.add_argument('-feedback_SNR2', type=int, default=100, help='100 means noiseless feeback') # feedback SNR for receiver 1
...
def update(alpha1, alpha2, rho, Dk, P, g, sigma1, sigma2):
	"""
	Update the alpha 1, alpha2, and rho
	"""
	alpha1_next = alpha1 * (P * g**2 * (1 - torch.square(rho))/Dk + sigma1**2)/(sigma1**2 + P)
	alpha2_next = alpha2 * (P * (1 - torch.square(rho))/Dk + sigma2**2)/ (sigma2**2 + P)

	A = P + sigma1**2 + sigma2**2
	B = (P + sigma1**2) * (P + sigma2**2)
	num = sigma1**2 * sigma2**2 * rho - (P * A * g * (1 - torch.square(rho))* sgn(rho) / Dk)
	den = B**(0.5) * torch.sqrt(P * g**2 * (1 - torch.square(rho))/Dk + sigma1**2) * torch.sqrt(P * (1 - torch.square(rho))/Dk + sigma2**2)
	rho_next = num/den
...
def ozarow(message1, forward_noise1, feedback_noise1, message2, forward_noise2, feedback_noise2, args):
	...
	Y12 = Y11 = X1  + forward_noise1[:,0].view(X1.shape[0], 1) # user1: ignore the second reception	
	Y22 = Y21 = X2  + forward_noise2[:,1].view(X2.shape[0], 1) # user2: ignore the first reception
	...
	for t in range(2, args.N + 1):
		if t == 2:
			...
			rho = torch.zeros(num_samples, 1)
			Dk = 1 + g**2 + 2 * g * torch.abs(rho)

			alpha1 = torch.var(epsilon1)
			alpha2 = torch.var(epsilon2)

		else:
			# t transmission(t >= 3)
			Xt = torch.sqrt(P * 1.0/ Dk) * (epsilon1/torch.sqrt(alpha1)  + epsilon2 * g * sgn(rho)/torch.sqrt(alpha2))
			
			X_total = torch.cat([X_total, Xt], dim = 1)
			# print(f"transmission: {t}, power: {torch.var(Xt)}")
			Y1t = Xt + forward_noise1[:,t - 1].view(X1.shape[0], 1)
			Y2t = Xt + forward_noise2[:,t - 1].view(X2.shape[0], 1)

			coe1 = (torch.sqrt(P/Dk) * torch.sqrt(alpha1) * (1 + g * torch.abs(rho)))/(P + sigma1**2)
			coe2 = (torch.sqrt(P/Dk) * torch.sqrt(alpha2) * (g + torch.abs(rho)) * sgn(rho))/(P + sigma2**2)
			

			theta1_estimate = theta1_estimate - Y1t * coe1
			theta2_estimate = theta2_estimate - Y2t * coe2
		
			# estimation error
			epsilon1 = theta1_estimate - theta1
			epsilon2 = theta2_estimate - theta2

			alpha1, alpha2, rho = update(alpha1, alpha2, rho, Dk, P, g, sigma1, sigma2)
			Dk = 1 + g**2 + 2 * g * torch.abs(rho)
